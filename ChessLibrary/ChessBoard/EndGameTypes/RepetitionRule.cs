...
internal class RepetitionRule : EndGameRule
{
    ...
    internal override bool IsEndGame()
    {
        ...
        if (movesCount >= MINIMUM_MOVES_COUNT && board.LastIrreversibleMoveIndex <= board.MoveIndex - MINIMUM_MOVES_COUNT) // If last 8 moves were reversible
        {
            var currentIndex = board.MoveIndex;

            HashSet<ChessBoard> piecesPositions = new HashSet<ChessBoard>(new ChessBoardComparer())
            {
                new ChessBoard(board.pieces, board.DisplayedMoves) { FenBuilder = board.FenBuilder, moveIndex = board.MoveIndex }
            };
